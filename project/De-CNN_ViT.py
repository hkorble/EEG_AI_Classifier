...
print("Number of test samples:", test_data_length)
...
class EEGClassifier(nn.Module):
    def __init__(self, num_classes=3):
        ...
        self.deconv = nn.Sequential(
            nn.ConvTranspose2d(1, 64, kernel_size=(3, 3), stride=2),
            nn.ReLU(),
            # Add more layers as necessary
        )
        self.reduce_channels = nn.Conv2d(64, 3, kernel_size=(1, 1))  # Reduce to 3 channels
...
for epoch in range(epochs):
    ...
    for data, labels in train_loader:
        ...
        runsThisEpoch += 1
        print("Runs this Epoch: ", runsThisEpoch)

    accuracy = 100 * total_correct / total_samples
    print(f'Epoch [{epoch+1}/{epochs}], Loss: {loss.item():.4f}, Accuracy: {accuracy:.2f}%')
    print("Time taken for this run: ", time.time() - trainingTimeThisEpochStart)

print("Total Training Time: ", time.time() - totalTrainingTimeStart)
...
print(f'Test Accuracy: {test_accuracy:.2f}%, Time taken for testing: {time.time() - testingTimeStart:.2f}s')